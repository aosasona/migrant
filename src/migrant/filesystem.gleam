...
import migrant/types.{
  ExpectedFolderError, ExtractionError, FileError, Migration, MigrationError,
  Migrations,
}
import migrant/lib
import simplifile
...
pub fn load_migration_files(
  ...
  parse_files(migrations_dir, files, map.new())
}

fn is_directory(
  path: String,
  next: fn(String) -> Result(Migrations, MigrationError),
) {
...
fn list_files(
  path: String,
  next: fn(List(String)) -> Result(Migrations, MigrationError),
) {
